...
podman create -it -v $PWD:$PWD:Z -v /var/lib/containers:/var/lib/containers --privileged -w $PWD --name centos-buildah centos:7 /bin/bash
podman start centos-buildah
podman exec centos-buildah bash -c "yum update -y && yum install buildah -y"
podman exec centos-buildah bash -c "./openjdk/openjdk-scratch.sh centos 7"
podman exec centos-buildah bash -c "./mule4/runtime/mule4-ee-scratch.sh centos 7 4.1.4"
